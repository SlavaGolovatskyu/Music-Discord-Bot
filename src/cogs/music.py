...
from src.utils.song_db import record_play, get_most_played
...
class Music(commands.Cog):
    ...
    async def automatic_play(self, ctx):
        ...
        # Record this play in the song database
        song_entry = Queue[ctx.guild.id][0]
        record_play(ctx.guild.id, song_entry['webpage_url'], song_entry['title'])
    ...
    @commands.command()
    async def mostplayed(self, ctx, limit: int = 10):
        """Show the most played songs on this server."""
        limit = max(1, min(limit, 25))  # clamp between 1 and 25
        top_songs = get_most_played(ctx.guild.id, limit=limit)

        if not top_songs:
            return await ctx.send("No songs have been played on this server yet!")
        ...
        for i, song in enumerate(top_songs, start=1):
            played_text = f"Played **{song['play_count']}** time{'s' if song['play_count'] != 1 else ''}"
            embed.add_field(
                name=f"{i}. {song['title']}",
                value=f"{played_text} — [link]({song['webpage_url']})",
                inline=False,
            )

        embed.set_footer(text=f"Top {len(top_songs)} songs on this server")
        await ctx.send(embed=embed)
